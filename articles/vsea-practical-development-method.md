---
title: "VSEA: 新しい開発手法で業務アプリを1日で実装した実践記録"
emoji: "🚀"
type: "tech"
...
| フェーズ | 時間 | 主な成果 |
|---------|------|---------|
| ① UX整合設計 | 0.5日 | ユーザーストーリー6件、画面遷移フロー、エラーケース5件 |
| ② スケルトン作成 | 0.5日 | 4モジュールのインターフェース定義 |
| ③-1 Slice 1 | 1.5日 | CSV読込・検証・GUI(540件処理成功) |
| ③-2 Slice 2 | 1日 | スタッフマスタ取得(103名取得成功) |
| ③-3 Slice 3 | 2日 | Playwrightスケジュール登録(dry_run成功) |
| ③-4 Slice 4 | 0.5日 | 不一致対応3択ダイアログ |
| ④ 肉付け | 1.5日 | 週単位ロジック、検証強化、UX改善 |
| パッケージング | 0.5日 | PyInstaller、README、技術仕様書 |
| **合計** | **8日** | **実稼働可能なアプリ完成** |

実質的な開発日数は**1日(8時間×8日 = 64時間)**程度でした。
